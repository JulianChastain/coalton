    ...
    (error, response) => {
      if (error) {
        ...
        sendErrorResponse(
          request,
          -32603,
          `Error fetching tools list: ${error.message}`,
          logger,
        );
        return;
      }
      ...
      try {
            ...
            inputSchema: {
              ...
              properties: {
                ...
                content: {
                  type: "string",
                  description:
                    "Alternative name for the request body content (for compatibility)",
                },
                headers: {
                  type: "object",
                ...
                rawResponse: {
                  type: "boolean",
                  description:
                    "If true, return the full response object instead of just the content",
                },
              },
              required: ["path"],
            },
            ...
            inputSchema: {
              ...
              properties: {
                id: {
                  type: "string",
                  description:
                    "Document ID to retrieve (e.g., 'claude-md', 'readme', 'yadd')",
                },
              },
              required: ["id"],
            },
            ...
            inputSchema: {
              ...
              properties: {
                name: {
                  type: "string",
                  description:
                    'The symbol name to look up (e.g., "map", "fold")',
                },
              },
              required: ["name"],
            },
        ...
        if (!toolsData.tools.some((t) => t.name === "list_definitions")) {
          toolsData.tools.push({
            ...
            description:
              "List all value definitions in the current Coalton environment",
            inputSchema: {
              type: "object",
              properties: {
                package: {
                  type: "string",
                  description:
                    'Optional package name to filter by (e.g., "COALTON-USER")',
                },
              },
              required: [],
            },
        ...
        if (!toolsData.tools.some((t) => t.name === "apropos_coalton")) {
          toolsData.tools.push({
            ...
            description:
              "Search for Coalton symbols matching a substring (case-insensitive)",
            inputSchema: {
              type: "object",
              properties: {
                search: {
                  type: "string",
                  description:
                    "The search string to match against symbol names",
                },
              },
              required: ["search"],
            },
        ...
        if (!toolsData.tools.some((t) => t.name === "reset_environment")) {
          toolsData.tools.push({
            ...
            description:
              "Reset the Coalton environment to its initial state (clears session definitions)",
            inputSchema: {
              type: "object",
              properties: {},
              required: [],
        ...
        if (!toolsData.tools.some((t) => t.name === "type_check_only")) {
          toolsData.tools.push({
            ...
            description:
              "Type-check a Coalton expression without evaluating it. Returns the inferred type.",
            inputSchema: {
              type: "object",
              properties: {
                code: {
                  type: "string",
                  description: "The Coalton expression to type-check",
                },
              },
              required: ["code"],
            },
        ...
        if (!toolsData.tools.some((t) => t.name === "multi_eval")) {
          toolsData.tools.push({
            name: "multi_eval",
            description:
              "Evaluate multiple Coalton/CL forms in sequence, returning results for each",
            inputSchema: {
              type: "object",
              properties: {
                code: {
                  type: "string",
                  description: "String containing multiple forms to evaluate",
                },
                package: {
                  type: "string",
                  description: "Optional package name for evaluation context",
                },
              },
              required: ["code"],
            },
        ...
        if (!toolsData.tools.some((t) => t.name === "describe_symbol")) {
          toolsData.tools.push({
            ...
            description:
              "Get comprehensive information about a Coalton symbol: type, type-def, class-def, and CL description",
            inputSchema: {
              type: "object",
              properties: {
                name: {
        ...
        if (!toolsData.tools.some((t) => t.name === "macroexpand_coalton")) {
          toolsData.tools.push({
            ...
            description:
              "Show the Common Lisp code generated by the Coalton compiler for an expression",
            inputSchema: {
              type: "object",
              properties: {
                code: {
                  type: "string",
                  description:
                    "The Coalton expression to compile and show generated code for",
                },
              },
              required: ["code"],
            },
        ...
        if (!toolsData.tools.some((t) => t.name === "disassemble_coalton")) {
          toolsData.tools.push({
            ...
            description:
              "Show the stored compiled code or disassembly for a Coalton function",
            inputSchema: {
              type: "object",
              properties: {
                name: {
                  type: "string",
                  description: "The function name to disassemble",
                },
              },
              required: ["name"],
            },
        ...
        if (!toolsData.tools.some((t) => t.name === "load_file")) {
          toolsData.tools.push({
            name: "load_file",
            description:
              "Load and evaluate a file containing Coalton/CL source code",
            inputSchema: {
              type: "object",
              properties: {
                path: {
                  type: "string",
                  description: "Absolute path to the source file to load",
                },
                package: {
                  type: "string",
                  description: "Optional package name for evaluation context",
        ...
        if (!toolsData.tools.some((t) => t.name === "shrubbery_eval")) {
          toolsData.tools.push({
            name: "shrubbery_eval",
            description:
              "Parse and compile Coalton code written in shrubbery/Rhombus notation (indentation-based infix syntax). Supports: def, fun, function calls, infix operators (+, -, *, /, >, <, ==, !=), and multi-line programs.",
            inputSchema: {
              type: "object",
              properties: {
                code: {
                  type: "string",
                  description:
                    'Shrubbery source code (e.g., "def add = fun(x, y): x + y")',
                },
              },
              required: ["code"],
            },
        ...
        if (!toolsData.tools.some((t) => t.name === "shrubbery_translate")) {
          toolsData.tools.push({
            name: "shrubbery_translate",
            description:
              "Show the Coalton S-expressions that a shrubbery snippet translates to, without compiling. Useful for understanding or debugging the translation.",
            inputSchema: {
              type: "object",
              properties: {
                code: {
                  type: "string",
                  description: "Shrubbery source code to translate",
                },
              },
              required: ["code"],
            },
        ...
        for (const tool of toolsData.tools) {
          const prefixed = createPrefixedToolName(
            config.SERVER_NAME,
            tool.name,
          );
        ...
        sendErrorResponse(
          request,
          -32603,
          `Error parsing tools list: ${parseError.message}`,
          logger,
        );
      }
    },
